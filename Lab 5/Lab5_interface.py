...
class Application(QMainWindow):
    ...
    def initUI(self):
        ...
        self.slave1_text.setStyleSheet("color: #ffffff; font-size: 20px;")
        self.slave1_text.setGeometry(10, label_top.height() + self.slave1_button.height() + 40, self.slave1_text.width(), self.slave1_text.height())

        self.slave1_error = QtWidgets.QLabel(self)
        self.slave1_error.setText("")
        self.slave1_error.setStyleSheet("color: #e80202; font-size: 18px;")
        self.slave1_error.setGeometry(10, label_top.height() + self.slave1_button.height() + self.slave1_text.height() + 40, self.slave1_error.width(), self.slave1_error.height())

        self.slave2_text = QtWidgets.QLabel(self)
        self.slave2_text.setText("Slave 2: ")
        self.slave2_text.setStyleSheet("color: #ffffff; font-size: 20px;")
        self.slave2_text.setGeometry(10, label_top.height() + self.slave1_button.height() + self.slave1_text.height() + self.slave1_error.height() + 40, self.slave2_text.width(), self.slave2_text.height())

        self.slave2_error = QtWidgets.QLabel(self)
        self.slave2_error.setText("")
        self.slave2_error.setStyleSheet("color: #e80202; font-size: 18px;")
        self.slave2_error.setGeometry(10, label_top.height() + self.slave1_button.height() + self.slave1_text.height() + self.slave1_error.height() + self.slave2_text.height() + 40, self.slave2_error.width(), self.slave2_error.height())
    ...
    def eventFilter(self, obj, event):
        ...
        if (event.type() == QtCore.QEvent.Type.MouseButtonPress and obj is self.slave1_button):
            ...
            try:
                ...
                if self.crc8(res) != self.crc8("Yatskiv Oleh Olehovych\0".encode("utf-8")):
                    self.slave1_error.setText("Error: corrupted data!")
                    self.slave1_error.adjustSize()
                else:
                    self.slave1_error.setText("")
                    self.slave1_error.adjustSize()
            except serial.serialutil.SerialException:
        ...
        if (event.type() == QtCore.QEvent.Type.MouseButtonPress and obj is self.slave2_button):
            ...
            try:
                ...
                if self.crc8(res) != self.crc8("11.09.2003\0".encode("utf-8")):
                    self.slave2_error.setText("Error: corrupted data!")
                    self.slave2_error.adjustSize()
                else:
                    self.slave2_error.setText("")
                    self.slave2_error.adjustSize()
            except serial.serialutil.SerialException:
